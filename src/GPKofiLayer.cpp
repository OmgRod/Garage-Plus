...
CCScene* GPKofiLayer::garageScene() {
    auto scene = CCScene::create();
    scene->addChild(GJGarageLayer::scene());
    return scene;
}

CCScene* GPKofiLayer::scene() {
    auto scene = CCScene::create();
    scene->addChild(GPKofiLayer::create());
    return scene;
}

GPKofiLayer* GPKofiLayer::create() {
...
bool GPKofiLayer::init() {
    ...
    auto title = CCLabelBMFont::create("Support OmgRod (Garage Plus)", "goldFont.fnt");
    title->setZOrder(1);
...
void GPKofiLayer::onKofiBtn(CCObject* sender) {
